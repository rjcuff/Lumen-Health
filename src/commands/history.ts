...
export async function runHistory(): Promise<void> {
  ...
  const byDate = new Map<string, {
    recovery?: (typeof records)[0];
    sleep?: (typeof records)[0];
    activity?: (typeof records)[0];
  }>();
  ...
  // Always include today so history never starts with a gap
  ...
  footer(['lumen', 'try: lumen ask "how did I sleep this week?"']);
}
